    ...
    for i in range(belief.shape[0]):
        for j in range(belief.shape[1]):
            ...
            if belief[i, j] > 0:
                ...
                i_new = int((d_t[i, j] - grid_spec["d_min"]) / grid_spec['delta_d'])  # replace with something that accounts for the movement of the robot
                j_new = int((phi_t[i, j] - grid_spec["phi_min"]) / grid_spec['delta_phi'])  # replace with something that accounts for the movement of the robot
